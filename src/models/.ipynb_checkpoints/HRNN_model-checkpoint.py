...
class Model:
    ...
    def build_model(self, 
        ...
        for _ in range(wavelet_levels):
            high_pass_res = conv1d(temp, self.weights['high_pass'], stride = 1)   
            temp = conv1d(temp, self.weights['low_pass'], stride = 2)   
            wavelet.append(high_pass_res)
        ...
        for i in reversed(range(wavelet_levels)):
            print wavelet_c[i], above_level_rep, i
