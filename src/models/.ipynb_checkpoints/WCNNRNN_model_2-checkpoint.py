...
class Model:
    ...
    def build_model(self, 
        ...
        #upsamples_stacked = tf.stack(upsamples, 1)
        #fused_CNN_wavelet = tf.reduce_mean(upsamples_stacked, 1)    
        #fused_CNN_wavelet.set_shape((None, None, weights_shape[-1][2]))
        
        fused_CNN_wavelet = tf.concat(upsamples, -1)  
        fused_CNN_wavelet.set_shape((None, None, wavelet_levels * weights_shape[-1][2]))
        
        #print (fused_CNN_wavelet)
        
        # RNN
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(lstm_n_hidden)    #, forget_bias=1.0)

        outputs, _ = tf.nn.dynamic_rnn(lstm_cell, fused_CNN_wavelet, dtype=tf.float32)   #output shape is [batch_size,t, lstm_n_hidden]
