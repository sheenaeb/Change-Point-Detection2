...
class Model:
    ...
    def build_model(self, 
        ...
        wavelet_shape = [self.n_variables, wavelet_kernel_size, 1, 1]
        ...
        antialiasfilter = tf.truediv(tf.ones(wavelet_shape),
                                     float(wavelet_kernel_size))
        
        self.weights['low_pass'] = tf.get_variable('low_pass', 
                                   dtype=tf.float32, 
                                   initializer=antialiasfilter,
                                   trainable=False,
                                  )
        
        self.weights['wfc'] = tf.get_variable('wfc', 
                                   shape=[lstm_n_hidden, self.n_classes],
                                   dtype=tf.float32, 
                                   initializer=tf.contrib.layers.xavier_initializer(),
        ...
        # wavelet 
        wavelet = []
        low_pass_res = tf.expand_dims(tf.transpose(self.x, (2, 0, 1)), -1)
       
        for _ in range(wavelet_levels):
            
            high_pass_res = tf.stack([
                conv1d(x, w, stride=1, activation=tf.nn.relu)
                for x, w in zip(tf.unstack(low_pass_res), tf.unstack(self.weights['high_pass']))
            ])
            
            low_pass_res = tf.stack([
                conv1d(x, w, stride=2)
                for x, w in zip(tf.unstack(low_pass_res), tf.unstack(self.weights['low_pass']))
            ])
        ...
        above_level_rep = tf.zeros((self.batch_size, tf.shape(wavelet_c[-1])[1], lstm_n_hidden))
        for i in reversed(range(wavelet_levels)):
            rnn_inputs = tf.concat((wavelet_c[i], above_level_rep), axis=2)
            rnn_inputs.set_shape((None, None, weights_shape[-1][2]+lstm_n_hidden))
            rnn_outputs, _ = tf.nn.dynamic_rnn(lstm_cell, rnn_inputs, dtype=tf.float32)   #output shape is [batch_size,t, lstm_n_hidden]
            
            above_level_rep = unpool(rnn_outputs, 2)
        
        
        #fused_CNN_wavelet.set_shape((None, None, wavelet_levels * weights_shape[-1][2]))
        

        # Linear activation, using rnn inner loop last output
        outputs_flat = tf.reshape (rnn_outputs, [-1, lstm_n_hidden])   
        mul_flat = tf.matmul(outputs_flat, self.weights['wfc']) + self.bias['bfc']
        lin_out = tf.reshape (mul_flat, [-1, tf.shape(wavelet_c[0])[1], self.n_classes])
