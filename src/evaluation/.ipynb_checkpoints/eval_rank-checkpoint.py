...
def non_max_supression(pred, window, data_num_step):
    '''
...
def worker_f(args):
    i, cpd, s_sorted, cpd_sorted, gt, tolerance = args
...
def parallel_ranking(cpd_val, cpd, gt, tolerance):
    '''
       pred: np.array 2d, probability of being classified in true class
       gt: np.array 2d, ground truth
    '''
    ...
    out = pool.map(worker_f, 
                   [(i, cpd, s_sorted, cpd_sorted, gt, tolerance) for i in range(1,len(sort_idx)+1)]
                  )
